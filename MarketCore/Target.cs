...
namespace MarketCore
{
    class Target
    {
        ...
        public string targetSearchBoxControl { get; set; }
        public string targetSearchBoxClick { get; set; }
        public string targetProductNameControl { get; set; }
        public string targetProductPriceControl { get; set; }
        public string targetMasterProductNameControl { get; set; }
        public string targetMasterProductPriceControl { get; set; }
        public string pageUrl { get; set; }
        public string pagelenght { get; set; }
        public List<SearchResults> targetSearchResults = new List<SearchResults>();
        public List<MasterProductList> targetMasterProductList = new List<MasterProductList>();

        public Target(string url)
        {
            // url comes from the ui

            MarketCoreControlReader mCoreControlReader = new MarketCoreControlReader("target");
            targetSearchBoxControl = mCoreControlReader.searchButton;
            targetSearchBoxClick = mCoreControlReader.searchClick;
            targetProductNameControl = mCoreControlReader.productName;
            targetProductPriceControl = mCoreControlReader.productPrice;
            targetMasterProductNameControl = mCoreControlReader.productMasterName;
            targetMasterProductPriceControl = mCoreControlReader.productMasterPrice;
        ...
        bool actionEnterProductName(string name)
        {
            try
            {
                var findsearchbox = iwebdriver.FindElement(By.Id(this.targetSearchBoxControl));
                findsearchbox.Clear();
        ...
        void actionClickSearchBox()
        {
            ...
            {
                //   var clicksearch = iwebdriver.FindElement(By.CssSelector(this.targetSearchBoxClick));
                //  clicksearch.Click();
            }
        ...
        string getProductNameFromSearchResults()
        {
            ...
            try
            {
                var resultTitle = iwebdriver.FindElement(By.CssSelector(this.targetProductNameControl));
        ...
        string getProductPrice()
        {
            try
            {
                var price = iwebdriver.FindElement(By.CssSelector(this.targetProductPriceControl));
                string targetReturnPrice = price.Text;
                if (targetReturnPrice.Contains("low"))
                    targetReturnPrice = "No Price";
                return targetReturnPrice;
            }
            catch (NoSuchElementException)
            {
        ...
        public bool searchProducts(string name)
        {
            ...
            targetSearchResults.Add(tempSearchResult);
            ...
            obj.priceTableUpdate("target", tempSearchResult.searchMasterString, tempSearchResult.searchResultName, tempSearchResult.searchResultPrice);
            Logger.log("--------------------");
            Logger.log(name);
            Logger.log("--------------------");
        ...
        public void createmasterlist()
        {
            ...
            {
                string newProductLink = this.targetMasterProductNameControl.Replace("child(x)", "child(" + i + ")");
                string newPriceLink = this.targetMasterProductPriceControl.Replace("child(x)", "child(" + i + ")");
                MasterProductList mp = new MasterProductList(i, getmasterProductname(newProductLink), getMasterProductPrice(newPriceLink));
                targetMasterProductList.Add(mp);
            ...
            foreach (var item in targetMasterProductList)
            {
                ...
                obj.priceTableUpdate("target", item.masterproductName, item.masterproductName, item.masterproductPrice);
